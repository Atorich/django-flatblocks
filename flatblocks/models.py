...
class BaseFlatBlock(models.Model):
    """
    ...
    slug = models.CharField(_('Slug'), max_length=255, unique=True,
                            help_text=_(
                                "A unique name used for reference in the templates")
                            )
    header = models.CharField(_('Header'), blank=True, max_length=255,
                              help_text=_(
                                  "An optional header for this content")
                              )
...
class FlatBlock(BaseFlatBlock):
    class Meta:
        verbose_name = _('Flat block')
        verbose_name_plural = _('Flat blocks')
        swappable = swapper.swappable_setting('flatblocks', 'FlatBlock')
